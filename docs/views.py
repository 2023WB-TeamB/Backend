...
class DocsShareView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if doc.url is not None:
            return Response({"message": "이미 URL이 생성된 문서입니다.", "status": 200, "share_url": doc.url},
                            status=status.HTTP_200_OK)
        ...
        doc.url = unique_url
        doc.save()
        return Response({"message": "URL이 생성되었습니다.", "share_url": unique_url}, status=status.HTTP_201_CREATED)
